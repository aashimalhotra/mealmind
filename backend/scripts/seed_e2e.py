...
def main():
    ...
    try:
        ...
        if not existing_e2e and recipe_count <= 10:
            ...
            print("E2E recipes already exist, skipping recipe seed.")
        ...
        # Create test meal plan if not exists
        meal_plan = session.query(MealPlan).filter_by(id='e2e-plan-1').first()
        if not meal_plan:
            meal_plan = MealPlan(
                id='e2e-plan-1',
            ...
            print("Meal plan e2e-plan-1 already exists, skipping.")
